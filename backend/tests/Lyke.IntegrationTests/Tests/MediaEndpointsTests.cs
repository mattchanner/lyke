...
public class MediaEndpointsTests : IClassFixture<LykeWebApplicationFactory>
{
    ...
    public async Task UploadMedia_WithCreatorToken_ReturnsSuccess()
    {
        ...
        await _client.PostAsJsonAsync(
            ...
            new RegisterCreatorRequest(DisplayName: "Upload Creator", Bio: null, SocialLinks: null)
        );
        ...
        var result = await response.Content.ReadFromJsonAsync<ApiResponse<MediaUploadResponse>>(
    ...
    public async Task UploadMedia_WithInvalidFileType_ReturnsBadRequest()
    {
        ...
        response
            .StatusCode.Should()
            .BeOneOf(HttpStatusCode.BadRequest, HttpStatusCode.InternalServerError);
    ...
    public async Task UploadMediaBulk_WithCreatorToken_ReturnsSuccess()
    {
        ...
        // Assert
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var result = await response.Content.ReadFromJsonAsync<ApiResponse<BulkMediaUploadResponse>>(
            LykeWebApplicationFactory.JsonOptions
        );
        result.Should().NotBeNull();
        result!.Success.Should().BeTrue();
    ...
    public async Task DeleteMedia_WithValidIds_ReturnsSuccess()
    {
        ...
        var uploadResult = await uploadResponse.Content.ReadFromJsonAsync<
        ...
        var response = await _client.SendAsync(
            new HttpRequestMessage(HttpMethod.Delete, "/api/media/v1/")
            {
    ...
    public async Task DeleteMedia_WithoutToken_ReturnsUnauthorized()
    {
        ...
        {
            MediaIds = new List<string> { Guid.NewGuid().ToString() },
        };
    ...
    public async Task GetSecureUrl_WithCreatorToken_ReturnsSecureUrl()
    {
        ...
        var response = await _client.GetAsync(
            $"/api/media/v1/secure-url?url={Uri.EscapeDataString(mediaUrl)}"
        );
