...
public class BodyProfileConfiguration : IEntityTypeConfiguration<BodyProfile>
{
    public void Configure(EntityTypeBuilder<BodyProfile> builder)
    {
        ...
        builder.Property(bp => bp.HeightCm).IsRequired();

        builder.Property(bp => bp.WeightKg).HasPrecision(5, 2).IsRequired();

        builder.Property(bp => bp.FitPreference).HasConversion<string>().HasMaxLength(20);

        builder.Property(bp => bp.Stature).HasConversion<string>().HasMaxLength(20);

        builder.Property(bp => bp.Build).HasConversion<string>().HasMaxLength(20);

        builder
            .HasOne(bp => bp.BodyType)
            .WithMany(bt => bt.BodyProfiles)
            .HasForeignKey(bp => bp.BodyTypeId)
            .OnDelete(DeleteBehavior.Restrict);

        builder
            .HasOne(bp => bp.FrameSize)
