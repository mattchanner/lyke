...
public class SocialTokenValidator : ISocialTokenValidator
{
    ...
    public async Task<SocialUserInfo> ValidateAsync(string provider, string idToken, CancellationToken cancellationToken = default)
    {
    ...
    private async Task<SocialUserInfo> ValidateGoogleTokenAsync(string idToken, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                throw new ValidationException("IdToken", "Google token does not contain an email claim");
            }
    ...
    private async Task<SocialUserInfo> ValidateAppleTokenAsync(string idToken, CancellationToken cancellationToken)
    {
        try
        {
            var jwksJson = await _httpClient.GetStringAsync(
                "https://appleid.apple.com/auth/keys", cancellationToken);
            ...
            {
                throw new ValidationException("IdToken", "Apple token does not contain required claims");
            }
